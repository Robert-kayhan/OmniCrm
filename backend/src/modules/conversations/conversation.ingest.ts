 ...
 */
async function resolveCustomerChannel(
  tx: Db,
...
export async function ingestNormalizedMessage(
  ...
  try {
    const outcome = await prisma.$transaction(async (tx) => {
      ...
      const { customerChannelId, customerId, customerCreated } = await resolveCustomerChannel(
        tx,
        integration,
        normalized,
      );

      const { conversationId, conversationCreated } = await resolveConversation(
        tx,
        integration,
        normalized,
        customerChannelId,
    ...
    if (outcome.created && outcome.message) {
      emitMessageCreated(integration.organizationId, outcome.message, outcome.conversation);
