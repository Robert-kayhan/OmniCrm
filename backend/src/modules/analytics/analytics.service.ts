...
async function messageVolume(
  ...
  const rows = await prisma.$queryRaw<Array<{ date: Date; inbound: bigint; outbound: bigint }>>`
    SELECT
      d.day::date AS date,
...
async function responseTimes(
  ...
  const [resolution] = await prisma.$queryRaw<Array<{ avg_resolution: number | null }>>`
...
async function messagesPerChannel(
  organizationId: string,
  from: Date,
  channels: string[] | undefined,
): Promise<Map<string, number>> {
  const rows = await prisma.$queryRaw<Array<{ channel: string; count: bigint }>>`
...
export async function getAnalyticsOverview(
  ...
  const [
    ...
    prisma.message.count({ where: { ...messageWindow, senderType: SenderType.CUSTOMER } }),
    prisma.message.count({
      where: { ...messageWindow, senderType: { not: SenderType.CUSTOMER } },
    }),
    prisma.customer.count({ where: { organizationId, createdAt: { gte: from } } }),
    prisma.conversation.count({
    ...
    prisma.conversation.aggregate({ where: liveScope, _sum: { unreadCount: true } }),
    ...
    messagesPerChannel(organizationId, from, channels),
    ...
    messageVolume(organizationId, from, channels),
    responseTimes(organizationId, from, channels),
    agentPerformance(organizationId, from),
  ]);
