...
import type { CursorMeta } from '../../utils/response';
import type { AuthContext } from '../../types/auth';
import { getProvider } from '../../channels';
import type { OutboundAttachment, SendMessageResult } from '../../channels';
import { emitConversationChanged, emitMessageCreated } from '../../realtime';
...
async function touchConversationOutbound(conversationId: string, sentAt: Date): Promise<void> {
 ...
 */
async function broadcastOutbound(organizationId: string, message: MessageDto): Promise<void> {
  ...
  emitMessageCreated(organizationId, message, conversation);
  if (conversation && !message.isInternal) {
    emitConversationChanged(conversation, 'message');
  }
}
...
export async function sendMessage(
  ...
  if (input.isInternal) {
    ...
    await broadcastOutbound(conversation.organizationId, created);
    return created;
  }
  ...
  const [credentials, customerChannel] = await Promise.all([
    getCredentialsForIntegration(conversation.organizationId, conversation.integrationId),
    prisma.customerChannel.findUnique({
  ...
  await touchConversationOutbound(conversationId, sentAt);
  await broadcastOutbound(conversation.organizationId, sent);
