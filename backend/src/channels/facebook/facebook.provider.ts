...
export class FacebookProvider implements MessagingProvider {
   ...
   * timing-safe, and a missing secret fails closed — an unverifiable delivery
   * is treated as forged rather than waved through.
   */
  verifyWebhookSignature({ rawBody, headers }: WebhookSignatureInput): boolean {
  ...
  /**
   * Answers Meta's subscription handshake, returning the challenge to echo
   * back or null when the token does not match.
   */
  verifySubscription(
